          ...
          <div className="grid grid-cols-7 gap-2">
            ...
            {hours.map((h) => (
              <>
                ...
                {days.map((d, idx) => {
                  const isClass = h === "18:00" && (idx === 0 || idx === 2);
                  if (isClass) {
                    return (
                      <div key={`${h}-${d}`} className="border rounded-xl bg-blue-50/50 border-blue-300 p-3 h-24 overflow-hidden">
                        <p className="text-xs text-gray-900 break-words leading-4">Piano Básico</p>
                        <span className="inline-flex items-center mt-2 text-xs border rounded-md px-2 py-1 max-w-full truncate">Sala 101</span>
                        <p className="text-xs text-gray-600 mt-1 break-words leading-4">12 estudiantes</p>
                      </div>
                    );
                  }
